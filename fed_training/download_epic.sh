...
"; then
    ...
    if [ -f "${LOCAL_DIR}/${CLIENT_NAME}.tar.gz" ]; then
        echo "Final tar.gz already exists locally; uploading to HF..."
        python3 -c "
...
"
    ...
    if [ -d "${LOCAL_DIR}/raw_epic_${participant}" ] && [ "$(ls -A ${LOCAL_DIR}/raw_epic_${participant}/*.MP4 2>/dev/null)" ]; then
        echo "  Raw videos already exist locally. Skipping download."
    else
      ...
      (
          ...
          for i in $(seq 0 $((num_segments - 1))); do
                ...
                ffmpeg -y -i "$raw" \
                    ...
                    "${LOCAL_DIR}/${CLIENT_NAME}/${base}_clip_%04d.mp4" 2>/dev/null
          done
          echo "     Finished $base"
      ) &
...
"
    ...
    echo "  Cleaning up local files..."
    rm -rf "${LOCAL_DIR}/raw_epic_${participant}" "${LOCAL_DIR}/${CLIENT_NAME}" "${LOCAL_DIR}/${CLIENT_NAME}.tar.gz"
done
